...
function handleClick(clickedButtonEvent) {
    ...
    let button_text = clickedButtonEvent.target.innerText;
...
function hideMoreText(card) {
    ...
    card.wrap.querySelector('.desc').replaceWith('');
...
function showMoreText(card) {
    ...
    let text = 'Стиральный порошок для стирки детского белья, 0.8 кг';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.wrap.querySelector('.productName').insertAdjacentHTML('beforeend', `<div class="desc">${text}</div>`);
